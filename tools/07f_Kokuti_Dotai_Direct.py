...
def draw(output_dir, scale=50, indices_override=None, start_draw_i=0, save=True, **kwargs):
    ...
    for draw_i, sec_i in enumerate(indices):
        ...
        exc_btm        = lp('exc_bottom')      # kiso 背面底
        exc_mid_btm    = lp('exc_mid_bottom')  # backfill_y 内側
        exc_mid_top_pt = lp('exc_mid_top')     # backfill_y 外側（= fi_btm）
        exc_top_pt     = lp('exc_top')         # 掘削上端（y == y_top）

        backfill_y = exc_mid_btm[1]            # == exc_mid_top_pt[1]
        ...
        if not has_gr:
            ...
            # ── GR基礎：小口止ポリゴンはブロック本体のみ ─────────
            c1 = (_x_at_y(blk_btm_local, tbtm_local, y_blk_top) + px, y_blk_top)
            c2 = (_x_at_y(blk_btm_local, tbtm_local, y_bottom)  + px, y_bottom)
            c3 = exc_btm
            c4 = exc_mid_btm
            c5 = exc_mid_top_pt
            c6 = (_x_at_y(exc_mid_top_pt, exc_top_pt, y_blk_top), y_blk_top)

            corners    = [c1, c2, c3, c4, c5, c6]
            y_rect_top = y_blk_top
            change_ys  = [backfill_y]   # x_r()がここで折れる（exc_mid_btm線→exc_mid_top線）＝幅変化点
            jt_y_top   = y_blk_top
            dim_change_ys = [backfill_y]
            ...
            # GR既製品 8頂点（yはワールド座標、0が天端高さ・y_blk_topが基礎コン底面に一致）
            gr_poly = [
                sg(0,           0),
                sg(400,         0),
                sg(400,         -(gr_h - 100)),
        ...
        boundaries = [jt_y_top] + joint_ys + [y_bottom]
