...
def draw(output_dir, scale=50, indices_override=None, start_draw_i=0, save=True, **kwargs):
    ...
    for draw_i, sec_i in enumerate(indices):
        ...
        # ペーライン背面 2点（danmen.dxf の 05_Foundation ポリラインから直接抽出）
        ...
        y_bottom    = pts['block_btm_front'][1]
        ...
        if not has_gr:
            ...
            y_top      = y_blk_top        # 打継目・鉄筋の上端（ブロック上面）
            ...
            change_ys  = [backfill_y]   # x_r()がここで折れる（掘削面→ぺーライン背面）＝幅変化点
        ...
        if not has_gr:
            # 天端コン下端（ブロック上面）境界線
            msp.add_line((fp, y_blk_top), (fp + kw, y_blk_top), dxfattribs=la)
