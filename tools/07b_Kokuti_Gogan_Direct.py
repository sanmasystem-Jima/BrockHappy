...
def draw(output_dir, scale=50, **kwargs):
    ...
    for draw_i, sec_i in enumerate(indices):
        ...
        if has_tc:
            ...
            if has_gr:
                # ── GR基礎：小口止ポリゴン = ブロック本体のみ（4頂点）──
                c1 = (_x_at_y(blk_btm_local, tbtm_local, y_blk_top) + px, y_blk_top)
                c2 = (_x_at_y(blk_btm_local, tbtm_local, y_bottom)  + px, y_bottom)
                c3 = (_x_at_y(exc_btm, exc_top, y_bottom), y_bottom)
                c4 = (_x_at_y(exc_btm, exc_top, y_blk_top), y_blk_top)
                corners    = [c1, c2, c3, c4]
                y_top      = y_blk_top   # 打継目・重心計算の起点をブロック上面に
                y_rect_top = y_blk_top
                change_ys  = []
                dim_change_ys = []   # x_r()に折れ点なし＝幅は変化しない
                ...
                c2 = (_x_at_y(blk_btm_local, tbtm_local, y_bottom) + px, y_bottom)
                c3 = (_x_at_y(exc_btm, exc_top, y_bottom), y_bottom)
                c4 = (_x_at_y(exc_btm, exc_top, y_top),    y_top)
                corners    = [c1, c2, c3, c4]
                y_rect_top = y_top
                change_ys  = [y_blk_top]
                dim_change_ys = []   # y_blk_topは前面のみの目印。x_r()に折れ点なし＝幅は変化しない

            def x_r(y):
                return _x_at_y(exc_btm, exc_top, y)

            def back_line(y):
                return exc_btm, exc_top
            ...
            # ── 天端コンなし：6頂点 ──────────────────────
            kiso_btm_y    = pts['kiso_btm_front'][1]
            saiseki_top_y = pts['saiseki_top_front'][1]
            ura_btm = lp('uracon_btm_back')
            ura_top = lp('tenba_btm_back')

            c1 = lp('tenba_btm_front')
            c2 = (_x_at_y(blk_btm_local, tbtm_local, kiso_btm_y) + px, kiso_btm_y)
            c3 = (_x_at_y(exc_btm, exc_top, kiso_btm_y), kiso_btm_y)
            c4 = (_x_at_y(exc_btm, exc_top, saiseki_top_y), saiseki_top_y)
            c5 = (_x_at_y(ura_btm, ura_top, saiseki_top_y), saiseki_top_y)
            c6 = lp('tenba_btm_back')
            corners    = [c1, c2, c3, c4, c5, c6]
            y_top      = c1[1]
            y_bottom   = c2[1]
            y_rect_top = y_top
            change_ys  = [saiseki_top_y]
            dim_change_ys = [saiseki_top_y]   # x_r()がここで折れる（裏コン背面→掘削面）＝幅変化点

            def x_r(y):
                if y > saiseki_top_y:
                    return _x_at_y(ura_btm, ura_top, y)
                else:
                    return _x_at_y(exc_btm, exc_top, y)

            def back_line(y):
                if y > saiseki_top_y:
                    return ura_btm, ura_top
                else:
                    return exc_btm, exc_top

            def sec_bnd(y_a, y_b):
                v = []
                if y_b < saiseki_top_y < y_a:
                    v += [(_x_at_y(ura_btm, ura_top, saiseki_top_y), saiseki_top_y),
                          (_x_at_y(exc_btm, exc_top, saiseki_top_y), saiseki_top_y)]
                return v
        ...
        # 縦鉄筋（打継目ごとに前後壁面から200mm内側・壁面平行・600mm）
